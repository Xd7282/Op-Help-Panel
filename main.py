    ...
    async def callback(self, interaction: discord.Interaction):
        if self.invoker == interaction.user:
            ...
            await interaction.response.send_message(
                "<:cancel:1067829890482712756> | Its not your help command menu!", ephemeral=True)


class Btns(discord.ui.Button):
...
class Buttons(discord.ui.Button):
    ...
    def __init__(self,
        ...
        if label == None:
          ...
          super().__init__(label=label, style=style, url=url, disabled=disable)
...
class View(discord.ui.View):
    ...
    def add_buttons(self):
        self.startB = Buttons(label="≪", emoji=None,
                              style=discord.ButtonStyle.blurple,
                              command=self.set_page,
                              args=0,
                              ctx=self.ctx)
        self.backB = Buttons(label="Back", emoji=None,
                             style=discord.ButtonStyle.success,
                             command=self.to_page,
                             args=-1,
                             ctx=self.ctx)
        self.nextB = Buttons(label="Next", emoji=None,
                             style=discord.ButtonStyle.success,
                             command=self.to_page,
                             args=+1,
                             ctx=self.ctx)
        self.endB = Buttons(label="≫", emoji=None,
                            style=discord.ButtonStyle.blurple,
                            command=self.set_page,
                            args=len(self.options) - 1,
                            ctx=self.ctx)
        self.quitB = Buttons(label=None, emoji="<a:crss_1:1072764257978032218>",
    ...
    def gen_embeds(self):
        ...
        options.append(
            discord.SelectOption(label="Index",
                                 emoji='<:emoji_49:1082127632705007698>', description="Shows Index Page"))
        embeds.append(self.home)
        for cog in self.get_cogs():
            if "help_custom" in dir(cog):
                emoji, label, description = cog.help_custom()
                options.append(discord.SelectOption(label=label, description=description, emoji=emoji))
                embed = discord.Embed(title=f"{label} Commands {emoji}",
                                      color=0x2f3136)
                embed.set_footer(
                    text="Made with 💖 By Team Apolex",
